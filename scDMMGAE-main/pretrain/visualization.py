...
def visualize_tsne(model_path, data_name, save_dir='/data/home/wangchi/scDMMGAE-main/pretrain/figures'):
    """
    ...
    # 获取嵌入向量 - 改为使用forward获取z_tilde
    with torch.no_grad():
        _, _, _, _, _, z_tilde, _, _, _ = model(input_data, adjn_tensor)
        embedding = z_tilde.cpu().numpy()
    ...
    plt.title(f'{data_name} Pretrain t-SNE', fontsize=16)
    plt.xlabel('t-SNE 1', fontsize=12)
    plt.ylabel('t-SNE 2', fontsize=12)
    plt.legend(markerscale=2)
    
    plt.tight_layout()
    plt.savefig(f'{save_dir}/{data_name}_tsne_pretrain.png', dpi=300, bbox_inches='tight')
    print(f"可视化结果已保存到 {save_dir}/{data_name}_tsne_pretrain.png")
    plt.close()

if __name__ == "__main__":
    # 从命令行参数获取数据集名称，或使用默认值
    data_name = args.name
    model_path = args.pre_model_save_path or f'model/model_save_pretrain/{data_name}_pretrain.pkl'
