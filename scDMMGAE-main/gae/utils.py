...
def eva(y_true, y_pred, epoch=0, features=None):
    """
    评估聚类性能的函数
    """
    # 计算NMI和ARI（原有指标）
    ...
    # 如果提供了特征，计算轮廓系数
    sil = None
    if features is not None and len(np.unique(y_pred)) > 1:
        try:
    ...
    if sil is not None:
        print('          Silhouette {:.4f}'.format(sil))
    
    # 确保返回单个值而不是元组
    return nmi, ari, acc, ami, sil
...
def get_laplacian(adj, normalized=True):
    """计算图拉普拉斯矩阵"""
    if normalized:
        D = np.diag(np.sum(adj, axis=1))
        L = D - adj  # 未归一化的拉普拉斯矩阵
        D_inv_sqrt = np.linalg.inv(np.sqrt(D))
        L_normalized = np.dot(np.dot(D_inv_sqrt, L), D_inv_sqrt)  # 归一化的拉普拉斯矩阵
        return L_normalized
    else:
        D = np.diag(np.sum(adj, axis=1))
        return D - adj
...
def compute_laplacian_loss(features, laplacian):
    """计算拉普拉斯嵌入损失"""
    # L = D - A, 其中L是拉普拉斯矩阵，D是度矩阵，A是邻接矩阵
    # 最小化 Tr(X^T L X) 可以使得连接紧密的节点在嵌入空间中也靠近
    features_t = torch.transpose(features, 0, 1)
    lap_loss = torch.trace(torch.matmul(torch.matmul(features_t, laplacian), features))
    return lap_loss
...
def compute_distance_matrix(X, metric='euclidean'):
    ...
    if metric == 'euclidean':
        dist_matrix = euclidean_distances(X_np)
    elif metric == 'cosine':
        dist_matrix = cosine_distances(X_np)
    elif metric == 'manhattan':
        dist_matrix = manhattan_distances(X_np)
    else:
        raise ValueError(f"Unknown distance metric: {metric}")
    
    return torch.FloatTensor(dist_matrix).to(X.device)


def multi_distance_loss(features, adj, distance_metrics=['euclidean', 'cosine']):
    """使用多种距离度量的联合损失"""
    total_loss = 0
    ...
    for metric in distance_metrics:
        # 在当前距离度量下计算距离矩阵
        dist_matrix = compute_distance_matrix(features, metric)
        
        # 将距离转换为相似度（距离越小，相似度越高）
        sim_matrix = 1.0 / (1.0 + dist_matrix)
        
        # 计算与原始图结构的差异
        loss = F.mse_loss(sim_matrix, torch.FloatTensor(adj_np).to(features.device))
        total_loss += loss
    
    return total_loss / len(distance_metrics)


def get_adj_multi_metric(count, k=10, pca=50, metrics=["euclidean", "cosine", "manhattan"]):
    """
    使用多种距离度量构建邻接矩阵
    ...
    参数:
        count: 数据矩阵
        k: k近邻参数
        pca: PCA降维维度，设为None则不进行降维
        metrics: 距离度量列表
    
    返回:
        adj_list: 不同度量下的邻接矩阵列表
        adj_n_list: 不同度量下的归一化邻接矩阵列表
    """
    if pca:
        countp = dopca(count, dim=pca)
    else:
    ...
    for metric in metrics:
        A = kneighbors_graph(countp, k, mode="connectivity", metric=metric, include_self=True)
        adj = A.toarray()
        adj_n = norm_adj(adj)
        adj_list.append(adj)
        adj_n_list.append(adj_n)
    
    return adj_list, adj_n_list


def compute_embedding_adj(embeddings, k=10, metric='euclidean'):
    """
    从嵌入空间计算邻接矩阵
    ...
    # 计算k近邻图
    A = kneighbors_graph(emb_np, k, mode="connectivity", metric=metric, include_self=True)
    adj = A.toarray()
    
    # 归一化邻接矩阵
    adj_norm = norm_adj(adj)
    
    return torch.FloatTensor(adj_norm).to(embeddings.device)